...
## ----create_tible--------------------------------------------------------
iris.tib <- as_tibble(iris)     #as_tibble function from tidyverse, the same as data.frames in R
iris.tib
...
library(readr)    ## if we gonna use just one fucntion from the package, just write (   read::read.delim   )

system.time(genom<-read.delim("../genome.txt", header = TRUE, sep = "" ))
dim(genom)
